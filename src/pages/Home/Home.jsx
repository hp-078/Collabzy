...
const Home = () => {
  const { influencers, fetchInfluencers } = useData();
  ...
  // Top Influencers for Dome Gallery (real data from API)
  const topInfluencers = influencers.slice(0, 10).map((influencer) => ({
    id: influencer._id,
    name: influencer.name || influencer.user?.name || 'Creator',
    niche: Array.isArray(influencer.niche) && influencer.niche.length > 0
      ? influencer.niche.join(' & ')
      : 'Content Creator',
    avatar: influencer.avatar || influencer.instagramData?.profilePicture || influencer.youtubeData?.thumbnail || '',
    followers: formatFollowers(influencer.totalFollowers),
    engagement: `${Number(influencer.averageEngagementRate || 0).toFixed(1)}%`,
    rating: Number(influencer.averageRating || 0).toFixed(1),
    bio: influencer.bio || 'Verified creator on Collabzy ready for brand collaborations.',
    specialties: Array.isArray(influencer.niche) && influencer.niche.length > 0
      ? influencer.niche
