...
const campaignService = {
  ...
  getAllCampaigns: async (filters = {}) => {
    const response = await api.get('/campaign', { params: filters });
    return response.data;
  },
  ...
  getMyCampaigns: async (filters = {}) => {
    const response = await api.get('/campaign/my-campaigns', { params: filters });
    return response.data;
  },

  // Get eligible campaigns for logged-in influencer
  getEligibleCampaigns: async (filters = {}) => {
    const response = await api.get('/campaign/eligible', { params: filters });
    return response.data;
  },

  // Get recommended campaigns for logged-in influencer
  getRecommendedCampaigns: async (limit = 10) => {
    const response = await api.get('/campaign/recommended', { params: { limit } });
    return response.data;
  },
